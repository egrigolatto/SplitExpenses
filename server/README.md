...
1. **Supabase** → crear proyecto en región **US West (Oregon)** (misma que `region: oregon` de Render). Para la connection string usar el **shared pooler en Session mode** (Connect → Connection pooling, host `aws-0-*.pooler.supabase.com`, usuario `postgres.<ref>`, puerto **5432**) y agregarle `?sslmode=no-verify`. Razones: Render solo sale por IPv4 (la conexión directa `db.*.supabase.co` es IPv6-only; el add-on IPv4 es pago), el puerto 6543 (transaction mode) rompe sesiones (migraciones), y este server ya trae `pg.Pool` propio. No usar `?sslmode=require`: con `pg` 8.x ese modo **verifica la CA**, y el pooler de Supabase presenta un certificado self-signed que Node no confía → el arranque crashea con `SELF_SIGNED_CERT_IN_CHAIN`. **Seguridad:** `no-verify` cifra el tráfico pero no valida la identidad del servidor (riesgo MITM aceptable para free tier/hobby; para un producto real, endurecer a `?sslmode=verify-full` descargando el CA bundle de Supabase y exponiéndolo con `NODE_EXTRA_CA_CERTS=/path/al/ca.pem` en Render).
